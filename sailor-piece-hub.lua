...
do
    ...
    if not src then
        warn("[SailorHub] Rayfield failed to load. F9:")
...
local SWORD_COSTS={
    [1]={coins=2500,  gems=0},
    [2]={coins=250000,gems=150},
    [3]={coins=650000,gems=650},
}
local SWORD_NPC={
...
-- =====================
--   TWEEN 1200 studs/sec
--   No collision + 0.15s hang + no anticheat rollback
-- =====================
local function tweenToPosition(targetCFrame)
    ...
    local duration=math.max(dist/1200,0.05)
    ...
    -- Hang 0.15s to prevent anticheat rollback
    local hangEnd=tick()+0.15
    while tick()<hangEnd do
...
local function tweenToNPC(npcName)
    ...
    tweenToPosition(CFrame.new(pos+Vector3.new(0,3,3)))
    return true
end

local function tweenToQuestNPC(npcName)
    local pos=npcPositions[npcName]
    if not pos then return false end
...
local function tweenToVec(vec)
    tweenToPosition(CFrame.new(vec+Vector3.new(0,3,0)))
end
...
--   SWORD SYSTEM
-- =====================
local function equipSwordByName(swordName)
    pcall(function()
    ...
    local hum=char and char:FindFirstChildOfClass("Humanoid")
    if bp and hum then
        local tool=bp:FindFirstChild(swordName)
        ...
        for _,t in ipairs(container:GetDescendants()) do
            if t:IsA("Tool") then
                for tier,name in pairs(SWORD_NAMES) do
                    if t.Name==name and tier>best then best=tier end
                end
            end
        end
    end
...
local function equipBestOwnedSword()
    ...
    local function scan(container)
        ...
        for _,t in ipairs(container:GetChildren()) do
            if t:IsA("Tool") then
                for tier,name in pairs(SWORD_NAMES) do
                    if t.Name==name and tier>bestTier then
                        bestTier=tier bestName=name
                    end
                end
            end
        end
    ...
    scan(lp:FindFirstChild("Backpack"))
    scan(char)
    if bestName and not characterHasSwordEquipped(bestTier) then
        equipSwordByName(bestName)
    end
end

local function equipFarmingLoadout()
    equipBestOwnedSword()
    if not characterHasSwordEquipped(1) then
        local char=lp.Character
        if char then
            for _,t in ipairs(char:GetChildren()) do
...
--   READ MONEY / LEVEL
-- =====================
local function readMoneyGems()
    local ls=lp:FindFirstChild("leaderstats")
    local coins,gems=0,0
    if ls then
        for _,v in ipairs(ls:GetChildren()) do
            local n=string.lower(v.Name)
            if n:find("coin") or n:find("gold") or n:find("money") or n=="cash" then
                coins=tonumber(v.Value) or 0
            end
            if n:find("gem") then gems=tonumber(v.Value) or 0 end
        end
    end
    return coins,gems
end
...
local function readLevel()
    ...
    if ls then
        for _,v in ipairs(ls:GetChildren()) do
            local n=string.lower(v.Name)
            if n:find("level") or n=="lvl" or n=="lv" then
                local num=tonumber(v.Value)
                if num and num>=1 then return math.floor(num) end
            end
        end
    end
    return nil
...
local function tryAutoBuySword()
    ...
    local coins,gems=readMoneyGems()
    local currentTier=getOwnedSwordTier()
    if currentTier>=3 then return end
    local buyTier=nil
    for tier=currentTier+1,3 do
        local cost=SWORD_COSTS[tier]
        if coins>=cost.coins and gems>=cost.gems then buyTier=tier end
    end
    ...
    pcall(function()
        teleportToIsland(info.island)
        task.wait(0.5)
        if tweenToNPC(info.npc) then
            task.wait(0.3)
            local npcHRP=findNPCHRP(info.npc)
            local npcModel=npcHRP and npcHRP.Parent
            local usedPrompt=false
            if npcModel then
                for _,desc in ipairs(npcModel:GetDescendants()) do
                    if desc:IsA("ProximityPrompt") then
                        firePrompt(desc) usedPrompt=true task.wait(0.2) break
                    end
                end
            end
            if not usedPrompt then pressE() task.wait(0.2) end
            holdE(2.2)
        end
        task.wait(0.3)
    end)
...
task.spawn(function()
    while true do
        ...
        local lv=readLevel()
        if lv then
            local target=pickQuestForLevel(lv)
...
--   HOGYOKU / DEMONITE
-- =====================
local function collectSpecialFragment(fragCF,promptSearch)
    tweenToVec(fragCF.Position)
    task.wait(0.45)
    local pp=promptSearch()
    if not pp then return "skipped" end
    local hold=pp.HoldDuration
    pp.HoldDuration=0
    for _=1,5 do
        pcall(function() fireproximityprompt(pp) end)
...
local function runAutoHogyokuCollect()
    task.spawn(function()
        ...
        isInteracting=true floorHiding=false floorHidePivot=nil
        -- Go to Hogyoku NPC first
        Rayfield:Notify({Title="Hogyoku",Content="Going to Quest NPC...",Duration=2})
        teleportToIsland("HollowIsland")
        task.wait(0.5)
        tweenToQuestNPC("HogyokuQuestNPC")
        task.wait(0.3)
        local npcHRP=findNPCHRP("HogyokuQuestNPC")
        local npcModel=npcHRP and npcHRP.Parent
        if npcModel then
            for _,desc in ipairs(npcModel:GetDescendants()) do
                if desc:IsA("ProximityPrompt") then
                    firePrompt(desc) task.wait(0.2) break
                end
            end
        else pressE() end
        task.wait(0.3)
        -- Collect 6 fragments
        local collected,skipped,failed=0,0,0
        for i=1,6 do
            local cf=HOGYOKU_CF[i]
            if not cf then continue end
            Rayfield:Notify({Title="Hogyoku "..i.."/6",Content="Tweening...",Duration=1.5})
            local result=collectSpecialFragment(cf,function()
                for _,obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("ProximityPrompt") and obj.Parent then
                        local model=obj.Parent
                        if model.Name and model.Name:lower():find("hogyoku") then
                            return obj
                        end
                    end
                end
            ...
            if result=="collected" then collected=collected+1
            elseif result=="skipped" then skipped=skipped+1
        ...
        Rayfield:Notify({
            Title="Hogyoku Done!",
            Content=string.format("OK %d  Skip %d  Fail %d",collected,skipped,failed),
            Duration=4,
        })
    end)
end
...
local function runAutoDemoniteCollect()
    task.spawn(function()
        ...
        isInteracting=true floorHiding=false floorHidePivot=nil
        Rayfield:Notify({Title="Demonite",Content="Going to Academy...",Duration=2})
        teleportToIsland("Academy")
        task.wait(0.5)
        tweenToQuestNPC("AnosQuestNPC")
        task.wait(0.3)
        local npcHRP=findNPCHRP("AnosQuestNPC")
        local npcModel=npcHRP and npcHRP.Parent
        if npcModel then
            for _,desc in ipairs(npcModel:GetDescendants()) do
                if desc:IsA("ProximityPrompt") then
        ...
        local collected,skipped,failed=0,0,0
        for i=1,2 do
            local cf=DEMONITE_CF[i]
            if not cf then continue end
            Rayfield:Notify({Title="Demonite "..i.."/2",Content="Tweening...",Duration=1.5})
            local result=collectSpecialFragment(cf,function()
                for _,obj in ipairs(workspace:GetDescendants()) do
                    if obj:IsA("ProximityPrompt") and obj.Parent then
                        local model=obj.Parent
                        if model.Name and model.Name:lower():find("demonite") then
                            return obj
                        end
                    end
                end
...
task.spawn(function()
    while true do
        ...
        table.sort(p)
        local hs=table.concat(p,",")
        if hs~=lastHotbarSnapshot then lastHotbarSnapshot=hs currentTools=scanTools() end
        local sk=scanSkills() local ss=""
        for _,s in ipairs(sk) do ss=ss..s.key..":"..s.name end
    ...
    local function updateGUI()
        ...
        for _,s in ipairs(skills) do
            ...
            row.MouseButton2Click:Connect(function()
                ...
                Rayfield:Notify({Title="["..s.key.."] CD",Content=next_cd.."s",Duration=2})
                updateGUI()
            end)
        end
        if #skills>0 then
...
local function acceptQuest()
    ...
    if island then
        ...
        task.wait(0.5)
        local found=tweenToQuestNPC(selectedQuestNPC)
    ...
    if npcModel then
        for _,desc in ipairs(npcModel:GetDescendants()) do
            if desc:IsA("ProximityPrompt") then
                firePrompt(desc) prompted=true task.wait(0.15) break
            end
        end
    end
...
task.spawn(function()
    ...
    while true do
        ...
        if characterHasSwordEquipped(1) then continue end
        local char=lp.Character
        local hum=char and char:FindFirstChildOfClass("Humanoid")
        if not hum or hum.Health<=0 then continue end
            ...
            if bp then
                for _,t in ipairs(bp:GetChildren()) do
                    if t:IsA("Tool") and t.Name==toolName then
                        hum:EquipTool(t) task.wait(0.1) break
                    end
                end
            end
        end
...
task.spawn(function()
    while true do
        ...
        equipFarmingLoadout() lastZ=-math.huge isInteracting=false
        ...
        while autoQuestEnabled and not killCountReached and not forceQuestChange do
            ...
            if not h or h.Health<=0 then
                ...
                if mobIsland then teleportToIsland(mobIsland) end
                equipFarmingLoadout() lastZ=-math.huge
                ...
                while autoWorldBossEnabled and worldBossFarming do
                    ...
                    if not h or h.Health<=0 then
                        ...
                        if c and r and probe then
                            ...
                            breakVelocity() task.wait(0.25)
                        end
                        equipFarmingLoadout() lastZ=-math.huge missCount=0
                        continue
                    end
...
task.spawn(function()
    while true do
        ...
        if not target and worldBossFarming then
            target=getNearest(findByContains(worldBossTargetName or selectedWorldBossName),hrp.Position)
        end
    ...
    Callback=function(Value)
        if Value then
            task.spawn(function()
                ...
                mobFarming=true equipFarmingLoadout() lastZ=-math.huge
    ...
    Callback=function(Value)
        if Value then
            task.spawn(function()
                ...
                bossFarming=true equipFarmingLoadout() lastZ=-math.huge
...
FarmingTab:CreateSection("Info")
FarmingTab:CreateLabel("Open Hub → select [1] Style, [2] Style")
FarmingTab:CreateLabel("Click skill row to enable auto-use")
FarmingTab:CreateLabel("Right-click skill to cycle CD presets")

-- ===================== NPCs TAB =====================
local NPCsTab=Window:CreateTab("NPCs",4483362458)
NPCsTab:CreateSection("👤 Quick NPC Teleport")
NPCsTab:CreateLabel("Tween 1200 studs/sec | No collision | 0.15s hang")
...
AutoProgressTab:CreateLabel("Auto quest by level ladder + auto buy sword + auto equip")
AutoProgressTab:CreateLabel("Katana (2,500c) → Dark Blade (250Kc+150g) → Gryphon (650Kc+650g)")
...
AutoProgressTab:CreateSection("⚔️ Manual Sword Equip")
AutoProgressTab:CreateButton({
    Name="Equip Katana",
    Callback=function()
        equipSwordByName("Katana")
        Rayfield:Notify({Title="Equipping",Content="Katana",Duration=2})
    end,
})
AutoProgressTab:CreateButton({
    Name="Equip Dark Blade",
    Callback=function()
        equipSwordByName("Dark Blade")
        Rayfield:Notify({Title="Equipping",Content="Dark Blade",Duration=2})
    end,
})
AutoProgressTab:CreateButton({
    Name="Equip Gryphon",
    Callback=function()
        equipSwordByName("Gryphon")
        Rayfield:Notify({Title="Equipping",Content="Gryphon",Duration=2})
    end,
})
AutoProgressTab:CreateButton({
    Name="Equip Best Owned Sword",
    Callback=function()
        equipBestOwnedSword()
        Rayfield:Notify({Title="Equipping",Content="Best owned sword",Duration=2})
    end,
})

Rayfield:Notify({
    Title="Sailor Piece Hub v55",
    Content="GitHub loader + World Boss + Hogyoku/Demonite + sword fix!",
    Duration=4,
})
