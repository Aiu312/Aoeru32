        ...
        function Impl.toolSwordTier(tool)
            ...
            if string.find(tool.Name:lower(),"combat",1,true) then return 0 end
        ...
        function Impl.tryGameEquipWeaponByName(weaponName,wantTierMin,maxAttempts)
            ...
            for _=1,maxAttempts do
                ...
                local hum=char and char:FindFirstChildOfClass("Humanoid")
                local bp=lp:FindFirstChild("Backpack")
                local tg=(bp and bp:FindFirstChild(weaponName,true)) or (char and char:FindFirstChild(weaponName,true))
                if hum and tg and tg:IsA("Tool") then pcall(function() hum:EquipTool(tg) end) end
                task.wait(0.075)
                if Impl.characterHasSwordEquipped(wantTierMin) then return true end
            end
        ...
        function Impl.moveSwordToolToBackpack(tool)
            ...
            if char and tool.Parent==char then return end
        ...
        function Impl.applySwordEquipPhases(tool,wantTierMin,options)
            ...
            local function pulse()
                ...
                Impl.tryRequestInventoryRemote()
                task.wait(0.05)
                if Impl.tryGameEquipWeaponByName(tool.Name,wantTierMin,6) then return true end
